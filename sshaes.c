...
#define LASTWORD(i) ( newstate[i] = ((unsigned long)Sbox[(block[i] >> 24) & 0xFF] << 24) | \
			    ((unsigned long)Sbox[(block[(i+C1)%Nb] >> 16) & 0xFF] << 16) | \
			    ((unsigned long)Sbox[(block[(i+C2)%Nb] >>  8) & 0xFF] <<  8) | \
			    ((unsigned long)Sbox[(block[(i+C3)%Nb]      ) & 0xFF]      ) )
...
{
    unsigned short i;
    static const short C1 = 1, C2 = 2, C3 = 3, Nb = 4;
...
#define LASTWORD(i) (newstate[i] = ((unsigned long)Sboxinv[(block[i] >> 24) & 0xFF] << 24) | \
			   ((unsigned long)Sboxinv[(block[(i+C1)%Nb] >> 16) & 0xFF] << 16) | \
			   ((unsigned long)Sboxinv[(block[(i+C2)%Nb] >>  8) & 0xFF] <<  8) | \
			   ((unsigned long)Sboxinv[(block[(i+C3)%Nb]      ) & 0xFF]      ) )
...
{
    unsigned short i;
    static const short C1 = 4 - 1, C2 = 4 - 2, C3 = 4 - 3, Nb = 4;
 ...
 * (256-bit).
 */
void aes_setup(AESContext * ctx, unsigned short blocklen,
...
static void aes_encrypt_cbc(unsigned char *blk, unsigned short len, AESContext * ctx)
{
word32 iv[4];
unsigned short i;
...
static void aes_decrypt_cbc(unsigned char *blk, unsigned long len, AESContext * ctx)
{
...
void aes128_sckey(unsigned char *key)
{
    aes_setup(&scctx, 16, key, 16);
}

void aes_csiv(unsigned char *iv)
{
unsigned short i;
...
void aes_sciv(unsigned char *iv)
{
unsigned short i;
