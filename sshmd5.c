...
void MD5Init(void)
{
    MD5_Core_Init(&md5c.core);
    md5c.blkused = 0;
    md5c.lenhi = md5c.lenlo = 0;
}

void MD5Update(char const *p, unsigned len)
{
     ...
     */
    md5c.lenlo += lenw;
    md5c.lenhi += (md5c.lenlo < lenw);

    if (md5c.blkused + len < BLKSIZE) {
	/*
	 * Trivial case: just add to the block.
	 */
	memcpy(md5c.block + md5c.blkused, q, len);
	md5c.blkused += len;
    } else {
	/*
	 * We must complete and process at least one block.
	 */
	while (md5c.blkused + len >= BLKSIZE) {
	    memcpy(md5c.block + md5c.blkused, q, BLKSIZE - md5c.blkused);
	    q += BLKSIZE - md5c.blkused;
	    len -= BLKSIZE - md5c.blkused;
	    /* Now process the block. Gather bytes little-endian into words */
	    for (i = 0; i < 16; i++) {
		wordblock[i] =
		    (((uint32) md5c.block[i * 4 + 3]) << 24) |
		    (((uint32) md5c.block[i * 4 + 2]) << 16) |
		    (((uint32) md5c.block[i * 4 + 1]) << 8) |
		    (((uint32) md5c.block[i * 4 + 0]) << 0);
	    }
	    MD5_Block(&md5c.core, wordblock);
	    md5c.blkused = 0;
	}
	memcpy(md5c.block, q, len);
	md5c.blkused = len;
    }
}

void MD5Final(unsigned char output[16])
{
    int i;
    unsigned pad;
    unsigned char c[64];
    ...
    if (md5c.blkused >= 56)
	pad = 56 + 64 - md5c.blkused;
    else
	pad = 56 - md5c.blkused;

    lenhi = (md5c.lenhi << 3) | (md5c.lenlo >> (32 - 3));
    lenlo = (md5c.lenlo << 3);
    ...
    for (i = 0; i < 4; i++) {
	output[4 * i + 3] = (md5c.core.h[i] >> 24) & 0xFF;
	output[4 * i + 2] = (md5c.core.h[i] >> 16) & 0xFF;
	output[4 * i + 1] = (md5c.core.h[i] >> 8) & 0xFF;
	output[4 * i + 0] = (md5c.core.h[i] >> 0) & 0xFF;
    }
}
